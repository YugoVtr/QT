...
void CadastroUsuario::on_pushButton_Cadastrar_clicked()
{
    try
    {
        ...
        Vitor::Usuario_Persistencia user("../Sibi_Vitor/Arquivos/Dados/usuarios.txt");
        user.incluir(usuario);

        Vitor::Telefone_Persistencia telefone("../Sibi_Vitor/Arquivos/Dados/telefones.txt");
        while(!telefones.isEmpty())
        {
            telefone.incluir(telefones.front());
            telefones.pop_front();
        }
        Vitor::Endereco_Persistencia endereco("../Sibi_Vitor/Arquivos/Dados/enderecos.txt");
