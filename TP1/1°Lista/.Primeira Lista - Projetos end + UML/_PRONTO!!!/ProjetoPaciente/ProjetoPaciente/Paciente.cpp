...
namespace PrimeiraLista{//inicio_nameSpace
    ...
    void Paciente::setPeso(int pesoP){
        if(pesoP<0)throw std::string(" PESO NAO PODE ASSUMIR VALOR NEGATIVO ");
        peso=pesoP;
    }
    void Paciente::setAltura(float alturaP){
        if(alturaP<0)throw std::string(" ALTURA NAO PODE ASSUMIR VALOR NEGATIVO ");
        altura=alturaP;
    }
    std::string Paciente::calcularFaixaDePeso()const{
        std::string faixaDePeso = " PESO NORMAL ";
