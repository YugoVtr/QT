...
{//Inicio_namespace
        ...
        numeroDeFilhos(0)
    {
     ...
     void Funcionario::setSalarioDoMes(float salarioDoMesP){
        if(salarioDoMesP<0) throw std::string (" SALARIO NAO PODE ASSUMIR VALOR NEGATIVO ");
        salarioDoMes=salarioDoMesP;
    }
     void Funcionario::setNumeroDeFilhos(int numeroDeFilhosP){
        if(numeroDeFilhosP<0) throw std::string (" NUMERO DE FILHOS NAO PODE ASSUMIR VALOR NEGATIVO ");
