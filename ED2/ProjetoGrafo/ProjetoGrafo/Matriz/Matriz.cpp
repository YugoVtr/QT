...
Matriz::Matriz(int quantidadeDeLinhas, int quantidadeDeColunas):
    ...
    ponteiro_Matriz(0)
{
    if(quantidadeDeLinhas<=0) throw std::string("Quantidade de Linhas deve ser > 0");
    if(quantidadeDeColunas<=0) throw std::string("Quantidade de Colunas deve ser > 0 ");
    try {ponteiro_Matriz = new int[quantidadeDeLinhas*quantidadeDeColunas];}
    catch(std::bad_alloc&){throw std::string("Sem memoria para criar matriz");}
...
{
    if(linha<0 || linha >= quantidadeDeLinhas) throw std::string("Linha fora do intervalo valido");
    if(coluna<0 || coluna >= quantidadeDeColunas) throw std::string("Coluna fora do intervalo valido");
...
Matriz* Matriz::operator + (Matriz const * const matriz)const
{
    if( quantidadeDeLinhas!=matriz->getQuantidadeDeLinhas() || quantidadeDeColunas!=matriz->getQuantidadeDeColunas())
        throw std::string("Nao pode ser adicionadas matriz de tamanhos diferentes");
    ...
    catch(std::bad_alloc&){throw std::string("Sem memoria para criar a matriz");}
    catch(std::string){throw std::string("Matriz nao Criada, nao podemos adicionar as matrizes");}
}
Matriz* Matriz::operator - (Matriz const * const matriz)const
{
    if( quantidadeDeLinhas!=matriz->getQuantidadeDeLinhas() || quantidadeDeColunas!=matriz->getQuantidadeDeColunas())
        throw std::string("Nao pode ser subtraidas matriz de tamanhos diferentes");
    ...
    catch(std::string){throw std::string("Matriz nao Criada, nao podemos subtrair as matrizes");}
}
Matriz* Matriz::operator * (Matriz const * const matriz)const
{
    if(this->quantidadeDeColunas != matriz->quantidadeDeLinhas) //Se as matrizes não podem ser multiplicadas
        throw std::string ("Matrizes não podem ser multiplicadas"); //Eu lanço um excessão
    try
    {
...
{
    if(expoente < 0 ) throw std::string ("E melhor não fazer divisão de matriz ainda");
    if(quantidadeDeLinhas!=quantidadeDeColunas) throw std::string ("Potenciação so de matriz quadrada");
    try
    {
...
//================================== Implementações mais recentes 08/05/2017 =====================================
Matriz* Matriz::zeros(int size){
    Matriz* zero = new Matriz(size,size);
    for(int i=0;i<size*size;i++)
        zero->ponteiro_Matriz[i]=0;
    return zero;
}
Matriz* Matriz::ones(int size){
    Matriz* one = new Matriz(size,size);
    for(int i=0;i<size*size;i++)
        one->ponteiro_Matriz[i]=1;
    return one;
}
Matriz* Matriz::eye(int size){
    Matriz* eye = new Matriz(size,size);
    for(int i=0;i<size;i++)
        for(int j=0;j<size;j++)
            eye->setElemento(i==j?1:0,i,j);
    return eye;
}
